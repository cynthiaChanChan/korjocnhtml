...
(function() {
	...
	MobileSelect.prototype = {
		...
		init: function(config){
			var _this = this; 
			_this.keyMap = config.keyMap ? config.keyMap : {id:'id', value:'geography', childs:'TreeList'};//value修改为geography
			_this.checkDataType();
		    ...
		    _this.ensureBtn.addEventListener('click',function(){
		    	...
		    	for(var i=0; i<_this.wheel.length; i++){
		    		i==_this.wheel.length-1 ? tempValue += _this.getInnerHtml(i) : tempValue +=  _this.connector;
		    	}
		    	if(_this.triggerDisplayData){

		    		//自己修改插入nodes
		    		if (config.trigger == ".startPlace" || config.trigger == ".endPlace") {
                       $(config.trigger).find("h3").html(tempValue);
		    		}else {
